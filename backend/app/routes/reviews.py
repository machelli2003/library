...
from app.models import Book, BorrowRecord, Review, User

reviews_bp = Blueprint("reviews", __name__)


@reviews_bp.get("/books/<path:book_id>/reviews")
def list_reviews(book_id):
    ...
    reviews = Review.objects(book_id=book_id).order_by("-created_at").all()
    return jsonify([r.to_dict() for r in reviews]), 200


@reviews_bp.get("/books/<path:book_id>/review-eligibility")
@jwt_required()
def check_eligibility(book_id):
    ...
    existing = Review.objects(user_id=user_id, book_id=book_id).first()
    if existing:
        return jsonify({
            "eligible": False,
...
@reviews_bp.post("/books/<path:book_id>/reviews")
@jwt_required()
